...
import pandas as pd
import torch
from LEGACY_build_vocab import WordVocab
...
def run_test(
    ...
    """Prepare a minimal legacy input folder for a single reference pair.

    This only creates filtered input files for the existing legacy logic.
    It does not change the inference logic itself.

    pair_copies defaults to 200 so legacy chunking (which assumes many pairs)
    does not produce an empty feature matrix.
    """
    project_root = Path(__file__).resolve().parents[1]
    model_dir = project_root / "data" / "for_SWAMP" / "models" / model_name
    ...
    if not smiles_source.exists():
        raise FileNotFoundError(f"Missing SMILES source file: {smiles_source}")
    if not sequence_source.exists():
        raise FileNotFoundError(f"Missing sequence source file: {sequence_source}")

    try:
        smiles_df = pd.read_csv(smiles_source)
    except Exception:
        smiles_df = pd.read_csv(smiles_source, sep=";")
    smiles_df = smiles_df[
    ...
    try:
        sequence_df = pd.read_csv(sequence_source)
    except Exception:
        sequence_df = pd.read_csv(sequence_source, sep=";")
    ...
    output_dir = (
        ...
        / "Kcat_predictions"
        / "_small_test"
        / f"legacy_{model_name}"
    )
    output_dir.mkdir(parents=True, exist_ok=True)

    for stale_file in [
        "protein_sequence_tensors.pkl",
        "SMILES_tensors.pkl",
        "final_per_gene_combination_results.json",
        "final_per_gene_combination_results.csv",
        "missing_genes_and_SMILES.csv",
    ]:
        stale_path = output_dir / stale_file
        if stale_path.exists():
            stale_path.unlink()
    for stale_path in output_dir.glob("per_gene_combination_results_*.json"):
        stale_path.unlink()
    for stale_path in output_dir.glob("per_gene_combination_results_*.csv"):
        stale_path.unlink()

    smiles_df.to_csv(output_dir / "final_SMILES_metabolite_df.csv", index=False)
    sequence_df.to_csv(output_dir / "final_transcript_sequence_df.csv", index=False)
    if pair_copies < 1:
        raise ValueError("pair_copies must be >= 1")
    pair_payload = {
        str(i): [reference_pair[0], reference_pair[1], []] for i in range(pair_copies)
    }
    with open(output_dir / "gene_smiles_reactions_pairs.json", "w") as f:
        json.dump(pair_payload, f, indent=2)

    print(f"Legacy small test input folder created at: {output_dir}")
...
if __name__ == "__main__":
    ...
    # Uncomment for the one-pair regression test input:
    combinations_output_location = run_test()
