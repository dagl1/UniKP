...
def evaluate(model, val_loader, vocab):
    ...
    for b, data in enumerate(val_loader):
        ...
        loss = F.nll_loss(
            output[1:].view(-1, len(vocab)), sm2[1:].contiguous().view(-1), ignore_index=PAD
        )
...
def main():
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.n_worker
    )
    val_dataset = Seq2seqDataset(args.test_data, vocab, is_train=False)
    val_loader = DataLoader(
        val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.n_worker
    )
    print(model)
